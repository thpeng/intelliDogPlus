...
public final class Rules {
	...
	public boolean isRulesConform(byte[] board, int [] startPos, int [] targetPos, byte playerOnTurn, boolean [] forHeaven){
		...
		for(int i = 0; i < startPos.length; i++){
			for(int j = 0; j < i; j++){
				if(targetPos[j] == startPos[i]){ //double move!
					return false;
				}
			}
...
//				assert false; //FIXME:find the caller that released the assert here!
	...
	/**
	 * This method will find all pieces of a player on the given board, that are
	 * not yet finished. (finished means no more movable -> at the end of heaven)
	 * @param board the board to search on
	 * @param player the player who's pieces shall be found
	 * @param card the card that will be used. 
	 * (With this information the set of movable pieces can be further reduced for some cards)
	 * @return an int[] containing all positions of the given player's pieces
	 */
	public int[] getMovablePiecesInGameForPlayer(byte[] board, byte player, int card){
		boolean isJack = card % 13 == Cards.HEARTS_JACK;
		boolean isFour = card % 13 == Cards.HEARTS_FOUR;
		boolean isSeven = card % 13 == Cards.HEARTS_SEVEN;
		int [] tempPieces = new int[4];
		int pieceCount = 0;
		int lastFinishedPos = 0;
		switch(player){
			case Players.P1:
				lastFinishedPos = 67;
				for(int i = lastFinishedPos; i >= 64; i--){
					if(board[lastFinishedPos] == Players.ANY_SAVE){
						lastFinishedPos -= 1;
					}
				}
				for(int i = lastFinishedPos; i >= 64; i--){
			...
			case Players.P2:
				lastFinishedPos = 71;
				for(int i = lastFinishedPos; i >= 68; i--){
					if(board[lastFinishedPos] == Players.ANY_SAVE){
						lastFinishedPos -= 1;
					}
				}
			...
			case Players.P3:
				lastFinishedPos = 75;
				for(int i = lastFinishedPos; i >= 72; i--){
					if(board[lastFinishedPos] == Players.ANY_SAVE){
						lastFinishedPos -= 1;
					}
				}
			...
			case Players.P4:
				lastFinishedPos = 79;
				for(int i = lastFinishedPos; i >= 76; i--){
					if(board[lastFinishedPos] == Players.ANY_SAVE){
						lastFinishedPos -= 1;
					}
				}
		...
		//count pieces
		for(int i = 63; i >= 0; i--){
			if(board[i] == Players.ANY_SAVE && !isJack){
				if(player == Players.P1 && i == HOMEPOSTION_P1){ 
					tempPieces[pieceCount++] = HOMEPOSTION_P1; 
					continue;
				}
				if(player == Players.P2 && i == HOMEPOSTION_P2){ 
					tempPieces[pieceCount++] = HOMEPOSTION_P2; 
					continue;
				}
				if(player == Players.P3 && i == HOMEPOSTION_P3){ 
					tempPieces[pieceCount++] = HOMEPOSTION_P3; 
					continue;
				}
				if(player == Players.P4 && i == HOMEPOSTION_P4){ 
					tempPieces[pieceCount++] = HOMEPOSTION_P4; 
					continue;
				}
			}
			if(board[i] == player){ //there is a piece of the player on the i-th field.
				if((!isJack && !isFour) && (i == 15 || i == 31 || i == 47 || i == 63)){
					if(i == 63){ //there is a piece on the field before P1's home field
						if(board[HOMEPOSTION_P1] == Players.ANY_SAVE){ //the home field is blocked
							if(isSeven && player == Players.P1){ //there is a seven, and the player is behind his own home field 
								tempPieces[pieceCount++] = 63; //then add the piece to the movable pieces
							} //otherwise do nothing!
						}else{
							tempPieces[pieceCount++] = 63; //the home field of P1 is not blocked, so add this piece to the movable ones
						}
					}
					if(i == 15){ //there is a piece on the field before P2's home field
						if(board[HOMEPOSTION_P2] == Players.ANY_SAVE){ //the home field is blocked
							if(isSeven && player == Players.P2){ //there is a seven, and the player is behind his own home field 
								tempPieces[pieceCount++] = 15; //then add the piece to the movable pieces
							} //otherwise do nothing!
						}else{
							tempPieces[pieceCount++] = 15; //the home field of P2 is not blocked, so add this piece to the movable ones
						}
					}
					if(i == 31){ //there is a piece on the field before P3's home field
						if(board[HOMEPOSTION_P3] == Players.ANY_SAVE){ //the home field is blocked
							if(isSeven && player == Players.P3){ //there is a seven, and the player is behind his own home field 
								tempPieces[pieceCount++] = 31; //then add the piece to the movable pieces
							} //otherwise do nothing!
						}else{
							tempPieces[pieceCount++] = 31; //the home field of P3 is not blocked, so add this piece to the movable ones
						}
					}
					if(i == 47){ //there is a piece on the field before P4's home field
						if(board[HOMEPOSTION_P4] == Players.ANY_SAVE){ //the home field is blocked
							if(isSeven && player == Players.P4){ //there is a seven, and the player is behind his own home field 
								tempPieces[pieceCount++] = 47; //then add the piece to the movable pieces
							} //otherwise do nothing!
						}else{
							tempPieces[pieceCount++] = 47; //the home field of P4 is not blocked, so add this piece to the movable ones
						}
					}
					continue; //we already added the piece if it was movable!
				}
				assert (pieceCount != 4);
				tempPieces[pieceCount++] = i;
			}
