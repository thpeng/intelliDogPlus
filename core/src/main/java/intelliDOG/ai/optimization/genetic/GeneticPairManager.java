...
/**
 * subalgorithm for the Genetic Algorithm with the pair extension
 * @author tpeng
 *
 */
public class GeneticPairManager extends GeneticManager 
{
private Properties randomSeedt2; 
	...
	public List<EvaluatorWrapper> loadPropertiesFromDirectory(String path) throws IOException
	{
		...
		List<EvaluatorWrapper> tier2 = super.loadPropertiesFromDirectory(path+"/partner");
		if(tier1.size() != tier2.size())
			throw new IllegalArgumentException("The subdirectory partner must contain the same number of properties!");
	...
	public void dumpAll(String path)
	{
		...
		{
			File f1 = new File(path+"/partner"); 
			if(!f1.exists())
			{
				f1.mkdirs(); 
			}
			ew.getMate().dump(path+"/partner", getGeneration());
		}
	}
}
